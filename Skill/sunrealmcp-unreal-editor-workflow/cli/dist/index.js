...
async function collectModulePaths(rootDir) {
...
async function executeCommand(definition, parsed) {
    ...
    const unrealCommand = definition.family === "raw" ? String(parsed.values.command) : String(definition.unrealCommand);
...
function parseOptions(tokens, parameters) {
    const values = {};
    const global = {
...
function parseTypedValue(parameter, rawValue) {
...
function cloneDefault(defaultValue, type) {
...
function parseNumber(rawValue, optionName) {
...
function parseJson(rawValue, optionName) {
    try {
...
function parseNumberTuple(rawValue, length, optionName) {
...
function printRootHelp(registry, jsonMode) {
...
function printFamilyHelp(registry, family, jsonMode) {
...
function printCommandHelp(registry, family, cliCommand, jsonMode) {
    const definition = registry.get(family, cliCommand);
    if (!definition) {
