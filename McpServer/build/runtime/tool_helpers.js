...
export function registerCommandTool(server, context, definition) {
    ...
    server.registerTool(name, { description }, async () => {
        ...
        const payload = definition.formatResponse?.(normalized, plainInput) ?? buildDefaultPayload(definition, normalized);
        return {
            content: [
                {
                    type: "text",
...
async function executeToolCommand(context, definition, input) {
...
async function executeTaskCommand(context, definition, command, params) {
...
async function pollTaskUntilCompletion(context, definition, taskId, acceptedResponse, session) {
    const taskOptions = definition.taskOptions ?? {};
...
function extractAcceptedTask(response, taskOptions) {
...
function normalizeTaskCompletion(statusResponse, taskId) {
    ...
    if (status === "failed" || status === "cancelled") {
        ...
        return {
            ...
            ok: false,
            error: {
                code: typeof embeddedError?.code === "string"
                    ? embeddedError.code
                    : status === "cancelled"
                        ? "TASK_CANCELLED"
                        : "TASK_FAILED",
                message: typeof embeddedError?.message === "string"
...
function buildDefaultPayload(definition, normalized) {
